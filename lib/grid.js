...
class Grid {
  ...
  configureWalls() {
    ...
    this.cells.forEach( (row, i) => {
      ...
      row.forEach( (cell, j) => {
        // westmost wall
        top.push(Bodies.rectangle(1, (i + 1) * 32 + 17, 2, 31,
          { isStatic: true,
            render: {
        ...
        // northmost wall
        bottom.push(Bodies.rectangle(j * 32 + 19, 32, 31, 2,
          { isStatic: true,
            render: {
             fillStyle: randomColor(),
             strokeStyle: randomColor(),
        ...
        if (!cell.isLinked(cell.east)) {
          top.push(Bodies.rectangle((j + 1) * 32 + 1, (i + 1) * 32 + 17, 2, 31,
            { isStatic: true,
              render: {
               fillStyle: randomColor(),
               strokeStyle: randomColor(),
        ...
        if (!cell.isLinked(cell.south)) {
          bottom.push(Bodies.rectangle(j * 32  + 19, (i + 2) * 32, 31, 2,
            { isStatic: true,
              render: {
               fillStyle: randomColor(),
               strokeStyle: randomColor(),
